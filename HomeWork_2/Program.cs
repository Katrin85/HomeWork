...
{
    int ten = number / 10;  // Отсекаем еденицы
    int des = ten % 10;   //  с помощью оператора остатка определяем десяток, который есть 2-я цифра
    return des;
}
...
{
    if (day == 6 || day == 7)  // условие. если день недели СБ или ВС
       Console.WriteLine("Это выходной день");  // выводим: выходной 
       
    if (day >= 1 && day <= 5)  // условие. если дни недели ПН - ПЯТ
       Console.WriteLine("Это рабочий день, подожди немного и будет выходной.");  // Выводим: будни
}
